...
class KShotDataLoader():
    ...
    def load_dataset(self):
        ...
        elif(self.args.dataset == 'soda'):
            return self.soda_load()
        elif(self.args.dataset == 'mydata'):
            return self.mydata_load()
    ...
    def soda_load(self):
        ...
        assert len(list(np.unique(train_images.targets))) == 5, 'train image targets length is not 5'
        split_by_label_dict = defaultdict(list)

        print('Load soda data finished.')
        for i in range(len(train_images.imgs)):
            split_by_label_dict[train_images.targets[i]].append(train_images.imgs[i])
        imgs = []
        targets = []
        ...
        string_classnames = utils.soda_classes()

        # For soda the test and val sets are the same -- hence returning same sets for both
        return train_images, train_loader, train_loader_shuffle, val_images, val_loader, val_images, val_loader, num_classes, string_classnames

    def mydata_load(self):
        ...
        assert len(list(np.unique(train_images.targets))) == 7, 'train image targets length is not 7'
        split_by_label_dict = defaultdict(list)

        print('Load mydata data finished.')
        for i in range(len(train_images.imgs)):
            split_by_label_dict[train_images.targets[i]].append(train_images.imgs[i])
        imgs = []
        targets = []
        ...
        string_classnames = utils.mydata_classes()

        # For mydata the test and val sets are the same -- hence returning same sets for both
        return train_images, train_loader, train_loader_shuffle, val_images, val_loader, val_images, val_loader, num_classes, string_classnames
